...
func (h *DocumentHandler) UpdateMeetingMinutes(c *gin.Context) {
	// Parse the multipart form
	if err := c.Request.ParseMultipartForm(10 << 20); err != nil { // 10 MB max
		utilities.ShowMessage(c, http.StatusBadRequest, "Unable to parse form: "+err.Error())
		return
	...
	meetingId := c.PostForm("meeting")
	if meetingId == "" {
		utilities.ShowMessage(c, http.StatusBadRequest, "Meeting id cannont be null")
		return
	}
	...
	err = h.documentService.UpdateMeetingMinutes(meetingId, fileURl, userID.(string))
	if err != nil {
		os.Remove(filepath)
		utilities.ShowMessage(c, http.StatusInternalServerError, err.Error())
		return
	...
	utilities.ShowMessage(c, http.StatusCreated, "minutes added successfully")
}
